...
export function createNarration({navigate}){
  ...
  function sync(){const lines=document.querySelectorAll(`[data-page="${current}"] .intro__roll > *`);let index=0;cues.forEach((t,i)=>{if(audio.currentTime>=t)index=i;});lines.forEach((line,i)=>line.classList.toggle('voice-current',i===index));}
  ...
  toggle.onclick=()=>audio.paused?play():audio.pause();wave.onclick=()=>audio.paused?play():audio.pause();controls.querySelector('.voice-replay').onclick=()=>{audio.currentTime=0;sync();play();};
  ...
  return {enter(id){++request;const ticket=request;audio.pause();cancelAnimationFrame(raf);current=id;error.textContent='';controls.hidden=!files[id]||id==='welcome';
